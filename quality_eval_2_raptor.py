...
import torch
from raptor import BaseSummarizationModel, BaseSummarizationModel, BaseQAModel, BaseEmbeddingModel, RetrievalAugmentationConfig, RetrievalAugmentation
...
class Llama3_1_70b_QA(BaseQAModel):
...
class Llama3_1_70b_Summarize(BaseSummarizationModel):
    def __init__(self, model="meta-llama/Meta-Llama-3.1:70B"):
        pass

    def summarize(self, context, question, max_tokens=150):
        try:
            input_text = f"Write a summary of the following, including as many key details as possible: {context}:"
            answer = get_ollama_response(input_text)
            return answer
        except Exception as e:
            # For some error that may happen
...
def qa_test():
    ...
    log_filename = f"raptor_eval_2_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
    # Llama3170b = Llama3_1_70b()
    # embedder = AlibabaEmbedding()
    RAC = RetrievalAugmentationConfig(summarization_model=Llama3_1_70b_Summarize(), qa_model=Llama3_1_70b_QA(), embedding_model=AlibabaEmbedding())
    RA = RetrievalAugmentation(config=RAC)
    # custom_config = RetrievalAugmentationConfig(tr_top_k=20, tr_num_layers=10, tr_start_layer=0,tb_max_tokens=2000,qa_model=Llama3170b, embedding_model=embedder,tree_builder_type="cluster")
